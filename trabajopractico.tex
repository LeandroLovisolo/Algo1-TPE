...
\begin{problema}{losMasFracasados}{j: JJOO, p: Pais}{[Atleta]}
	...
	\aux{noGanoYMedallasNoGanadas}{j: JJOO, a: Atleta}{\ent}{if\hspace{1mm} todos([\lnot(a \in [x| x\leftarrow[0..min(3, |ranking(c)|))]])| c\leftarrow competencias(j), finalizada(c)\hspace{1mm} \land \hspace{1mm} (a \in ranking(c)])\hspace{1mm}then\hspace{1mm} | [c | c\leftarrow competencias(j), finalizada(c) \land (a \in ranking(c))] |\hspace{1mm}else\hspace{1mm} 0}
\end{problema}
...
\aux{atletasIgualesExceptoUnAtletaQueSeCambia}{c1, c2: Competencia, a : Atleta}{bool}{\newline(V p1 <- participantes(c1))\newline
\hspace*{10mm}(V p1 <- participantes(c1))\newline
\hspace*{15mm}(E p2<-participantes(c2))\newline
\hspace*{20mm}if p1 != a then p2 == p1 else\newline \hspace*{25mm}atletaIgualConNacionalidadCambiada(p2, p1, p)}
...
\begin{problema}{SequiaOlimpica}{j: JJOO}{[Pais]}
	...
	\vspace{5mm}
	\aux{maximosDiasConsecutivosSinGanar}{j: JJOO, p: Pais}{\ent}{max([|victorias(j, p)[x..y]| \hspace{1mm} | x, y \leftarrow [0..|victorias(j, p)|), x \leq y, (\forall v \leftarrow victorias(j, p)[x..y])v == False])}
...
\begin{problema}{transcurrirDia}{j: JJOO}{}
	...
	\aux{rankingDeCompetencia}{c: Competencia}{\bool}{(|ranking(c)| > 1)\rightarrow(\forall i \leftarrow [0..|ranking(c)|-1))capacidad(ranking(c)[i], prm(c)) \geq capacidad(ranking(c)[i+1], prm(c))}
